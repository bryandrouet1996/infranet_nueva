                ...
                if (!adjuntoName.isEmpty()) {
                    ...
                    if (enlace.CrearMemorandum(memorandumInfo)) {
                        ...
                        if (enlace.actualizarAdjuntoMemorandumID(elemento.getId(), elemento)) {
                            part.write(path);
                            enlace.enviarCorreoHotmail(encargado.getCorreo(), "Estimado funcionario, su director " + nombre_asignador + " le ha asignado el memorandum " + memorandumInfo.getDocumento() + ",dicho documento debe ser resuelto hasta la fecha " + memorandumInfo.getFecha_limite() + " en (" + elemento.getDias_restantes() + ") DIAS, para más detalles ingrese al módulo de asignaciones.");
                            out.print(1);
                        }
                    } else {
                        out.print(-1);
