...
<%@page import="enums.CorreoMemorandumEnum"%>
...
<%
    ...
    ArrayList<Memorandum> listadoPorCaducar = null;
    ArrayList<Memorandum> listadoPendientes = null;
    ArrayList<Memorandum> listadoCompletados = null;
    try {
        ...
        listadoPorCaducar = enlace.ObtenerMemorandumsPorCaducarAsignadorID(informacion.getId_usuario());
        listadoPendientes = enlace.ObtenerListadoMemorandumsAsignadorIDEstado(informacion.getId_usuario(), StatusEnum.PENDIENTE.getParam());
        listadoCompletados = enlace.ObtenerListadoMemorandumsAsignadorIDEstado(informacion.getId_usuario(), StatusEnum.COMPLETADO.getParam());
    } catch (Exception e) {
...
<html lang="es">
    <head>
        ...
        <title>Intranet Alcaldía - Listado de memorandums</title>
        <!-- General CSS Files -->
                        ...
                        <div class="section-header">
                            ...
                            <div class="section-header-breadcrumb">
                                <div class="flex-column activities">
                                    <a href="sesion.control?accion=registro_memorandum_coordinador&iu=<%=informacion.getId_usuario()%>" class="btn btn-primary" > <i class="fas fa-plus"></i> Ingresar nuevo</a>   
                                </div>
                            </div>
                        </div>
                        <div class="section-body">
                            <div class="row">
                                <div class="col-12">
                                    <div class="card">
                                        <div class="card-body">
                                            <%if (!listadoPorCaducar.isEmpty()) {%>
                                            <div class="alert alert-warning alert-has-icon">
                                                <div class="alert-icon"><i class="far fa-lightbulb"></i></div>
                                                <div class="alert-body">
                                                    Estimado funcionario existen memorandums que usted asigno y estan a punto de cumplir su fecha límite, se detalla el número documento y los dias que le quedan:
                                                    <ul>
                                                        <%for (Memorandum memorandum : listadoPorCaducar) {
                                                                usuario usuarioInfo = enlace.buscar_usuarioID(memorandum.getId_asignado());
                                                                String nombre = usuarioInfo.getNombre().concat(" ").concat(usuarioInfo.getNombre());
                                                                if (memorandum.getDias_restantes() == 0) {
                                                                    if (memorandum.getNotificado() == 0) {
                                                                        enlace.enviarCorreoHotmail(CorreoMemorandumEnum.TALENTO_HUMANO.getParam(), "El funcionario " + nombre + " no cumplio con su asignación indicada en el memorandum N° " + memorandum.getDocumento());
                                                                        enlace.actualizarMemorandumNotificacion(memorandum.getId());
                                                                    }
                                                                }
                                                        %>
                                                        <li>
                                                            <%= memorandum.getDocumento()%> <span class="badge badge-light"><%= memorandum.getDias_restantes()%></span>
                                                        </li>
                                                        <%}%>
                                                    </ul>
                                                </div>
                                            ...
                                            <ul class="nav nav-tabs" id="myTab" role="tablist">
                                                <li class="nav-item">
                                                    <a class="nav-link active" id="pendientes-tab" data-toggle="tab" href="#pendientes" role="tab" aria-controls="pendientes" aria-selected="true"><i class="fas fa-arrow-circle-up"></i> Asignaciones</a>
                                                </li>
                                                <li class="nav-item">
                                                    <a class="nav-link" id="completados-tab" data-toggle="tab" href="#completados" role="tab" aria-controls="completados" aria-selected="false"><i class="fas fa-arrow-circle-down"></i> Asignaciones completadas</a>
                                                </li>
                                            </ul>
                                                ...
                                                <div class="tab-pane fade show active" id="pendientes" role="tabpanel" aria-labelledby="pendientes-tab">
                                                    <div class="table-responsive">
                                                        <table class="table table-striped" id="table-x">
                                                            ...
                                                            <tbody>
                                                                <%for (Memorandum memorandum : listadoPendientes) {
                                                                        usuario usuarioAsignado = enlace.buscar_usuarioID(memorandum.getId_asignado());
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td><span class="badge badge-primary"><%= memorandum.getDias_restantes()%></span></td>
                                                                        <%if (usuarioAsignador.getId_usuario() == id) {%>
                                                                    <td><%= usuarioAsignado.getNombre().concat(" ").concat(usuarioAsignado.getApellido())%></td>
                                                                    <%} else {%>
                                                                    <td><%= usuarioAsignador.getNombre().concat(" ").concat(usuarioAsignador.getApellido())%></td>
                                                                    <%}%>
                                                                    <td>
                                                                        <%if (!memorandum.getAdjunto().equals("NA")) {%> 
                                                                        <a href="AdjuntoMemorandumController.ct?opcion=inicial&id_memorandum=<%= memorandum.getId()%>" class="btn btn-primary btn-sm" title="Descargar"><i class="fas fa-paperclip"></i></a> 
                                                                            <%} else {%> 
                                                                        <a href="AdjuntoMemorandumController.ct?opcion=inicial&id_memorandum=<%= memorandum.getId()%>" class="btn btn-primary btn-sm disabled" title="Descargar"><i class="fas fa-paperclip"></i></a> 
                                                                        <%}%>                                                                    </td>
                                                                    <td><a class="badge bg-warning"><%= memorandum.getEstado()%></a></td>
                                                                        <%if (usuarioAsignador.getId_usuario() != id) {%>
                                                                    <td>
                                                                        <%if (memorandum.getDias_restantes() > 0) {%>
                                                                        <a href="javascript:" data-toggle="modal" data-target="#modalCompletar" data-memorandum="<%= memorandum.getId()%>" class="btn btn-primary btn-sm rounded-pill " title="Completado"><i class="fa fa-check-circle"></i></a>
                                                                            <%} else {%>
                                                                        <a href="javascript:" data-toggle="modal" data-target="#modalCompletar" data-memorandum="<%= memorandum.getId()%>" class="btn btn-primary btn-sm rounded-pill disabled" title="Completado"><i class="fa fa-check-circle"></i></a>
                                                                            <%}%>
                                                                    </td>
                                                                    <%} else {%>
                                                                    <td> <a href="javascript:"  class="btn btn-primary btn-sm rounded-pill " onclick="EliminarMemorandumID(<%= memorandum.getId()%>)" title="Completado"><i class="fa fa-trash"></i></a></td>
                                                                            <%}%>
                                                                </tr>
                                                                <%}%>
                                                            </tbody>
                                                ...
                                                <div class="tab-pane fade" id="completados" role="tabpanel" aria-labelledby="completados-tab">
                                                    <div class="table-responsive">
                                                        <table class="table table-striped" id="table-xx">
                                                            ...
                                                            <tbody>
                                                                <%for (Memorandum memorandum : listadoCompletados) {
                                                                        usuario usuarioAsignado = enlace.buscar_usuarioID(memorandum.getId_asignado());
                                                                        usuario usuarioAsignador = enlace.buscar_usuarioID(memorandum.getId_usuario());
                                                                %>
                                                                <tr>
                                                                    <td><%= memorandum.getFecha_update()%></td>
                                                                    <td><%= memorandum.getDocumento()%></td>
                                                                    <td><%= memorandum.getDescripcion()%></td>
                                                                    <%if (usuarioAsignador.getId_usuario() == id) {%>
                                                                    <td><%= usuarioAsignado.getNombre().concat(" ").concat(usuarioAsignado.getApellido())%></td>
                                                                    ...
                                                                    <%}%>
                                                                    <td>
                                                                        <a href="AdjuntoMemorandumController.ct?opcion=final&id_memorandum=<%= memorandum.getId()%>" class="btn btn-primary btn-sm" title="Descargar"><i class="fas fa-paperclip"></i></a>
                                                                    </td>
                                                                    <td><a class="badge bg-success"><%= memorandum.getEstado()%></a></td>
                                                                </tr>
                                                                <%}%>
                                                            </tbody>
                                                        </table>
                ...
                <div class="modal-dialog modal-lg" role="dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <p class="small"></p>
                            <form id="formCompletarDirector" action="memorandum.ct?opcion=completar" method="post" enctype="multipart/form-data" class="needs-validation">
                                <div class="form-row">
